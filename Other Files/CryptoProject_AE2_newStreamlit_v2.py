...
title_writing = "Crypto Currency Prediction"
title_format = f'<p style="text-align: center; font-family: ' \
               f'Arial; color: #9900FF; font-size: 50px; ' \
...
crypto_coin = st.sidebar.selectbox('Select Crypto Currency',cryptocoin_list)
currency_list = ['CAD','USD','GBP','EUR','INR', 'AUD','JPY','CNY']
...
dates1 = pd.date_range(start, end, freq='D')
dates_train = dates1[:split,]
dates_test = dates1[split:,]
...
plt.plot(test_t[:], recursive_predictions[1:,], marker = '.', color='blue',label='recursive_predictions Prices')               # keep uncommented only if recursive_predict above code uncommented on test_set
...
def predict(dt0,dt1):
    ...
    custom_test_data = yf.download(f'{crypto_coin}-{currency}', start=dt3, end=dt1, progress=False,)
    ...
    st.header('Prediction and Actual Prices for ' + crypto_coin + ' with selected date from ' + str(dt0) + ' to ' + str(dt1) + ' as following:')
    ...
    st.write('Prediction and Actual Prices for ' + crypto_coin + ' with selected date from ' + str(dt0) + ' to ' + str(dt1) + ' as following:', df)
    print('Prediction and Actual Prices for ' + crypto_coin + ' with selected date from ' + str(dt0) + ' to ' + str(dt1) + ' as following:')
    print(df)
...
def recursive_predict(dt2):
    ...
    if dt2 <= today:
        print('The chosen date is in past, Select any future date')
        st.write('The chosen recursive date is in past, Select any future date for recursive_prediction')
        return
    ...
    st.header('Recursive Prediction Prices for ' + crypto_coin + ' with selected recursive end date ' + str(dt2) + ' as following:')
    custom_test_dates = pd.date_range(start, dt2, freq='D')
    plt.figure(figsize=(16,8))
...
default_date_after_1_month = (dt.datetime.now() + dt.timedelta(days=15)).date()             # default dates mainly for streamlit date selector
recursive_date = st.sidebar.date_input('Provide a recursive_date', value= default_date_after_1_month)
recursive_date = recursive_date.strftime("%Y-%m-%d")    
# recursive_date = '2024-07-15'
recursive_predict(recursive_date)


# to predict crypto value betbeen custom_test daterange
default_start_date = '2024-04-01'                                 # default dates mainly for streamlit date selector
default_end_date = '2024-05-27'
start_date = st.sidebar.date_input('Provide a starting date', value= dt.datetime.fromisoformat(default_start_date).date())
end_date = st.sidebar.date_input('Provide an ending date', value= dt.datetime.fromisoformat(default_end_date).date())
